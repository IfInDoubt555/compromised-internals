<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
  <div class="w-full mx-auto px-4 sm:px-6 lg:px-8 max-w-full">
    ...
    {{-- ===================== ROW 1: Brand + Auth (left) | Admin + Theme (right) ===================== --}}
    <div class="flex items-center justify-between h-16">
      ...
      <div class="flex items-center gap-6 whitespace-nowrap">
        ...
        @else
          <div class="hidden sm:flex items-center gap-2 text-sm lg:text-base whitespace-nowrap">
      ...
      @php
        // Safe URL helpers: if a named route is missing (e.g., 503), fall back to a plain URL
        $url = function ($name, $fallback) {
          return \Illuminate\Support\Facades\Route::has($name) ? route($name) : url($fallback);
        };
      @endphp

      {{-- Right: Admin (if allowed) + Theme toggle + Hamburger (mobile only) --}}
      <div class="flex items-center gap-4">

        @can('access-admin')
          <x-nav-link href="{{ route('admin.dashboard') }}" :active="request()->routeIs('admin.*')" class="hidden lg:inline-flex whitespace-nowrap">
            Admin Panel
          </x-nav-link>
        @endcan

        {{-- Theme Switch (always far right on desktop) --}}
        <button
          x-data
        ...
        <button @click="open = ! open" class="p-2 text-gray-400 hover:text-gray-500 hover:bg-gray-100 rounded-md focus:outline-none lg:hidden">
    ...
    {{-- ===================== ROW 2 (DESKTOP ONLY): Centered main nav ===================== --}}
    <div class="hidden lg:flex items-center justify-center h-12">
      <nav class="flex items-center gap-6 text-sm lg:text-base whitespace-nowrap">
        <x-nav-link href="{{ route('home') }}" :active="request()->routeIs('home')">Home</x-nav-link>

        <x-nav-link href="{{ $url('calendar.index','/calendar') }}"
                    :active="request()->routeIs('calendar.index')">Rally Schedule</x-nav-link>

        <x-nav-link href="{{ $url('travel.plan','/travel/plan') }}"
                    :active="request()->routeIs('travel.plan')">Plan Your Trip</x-nav-link>

        <x-nav-link href="{{ $url('history.index','/history') }}"
                    :active="request()->routeIs('history.*')">History</x-nav-link>

        <x-nav-link href="{{ $url('blog.index','/blog') }}"
                    :active="request()->routeIs('blog.index')">Blog</x-nav-link>

        <x-nav-link href="{{ $url('resources','/rally-resources') }}"
                    :active="request()->routeIs('resources')">Rally Resources</x-nav-link>

        <x-nav-link href="{{ $url('shop.index','/shop') }}"
                    :active="request()->routeIs('shop.index') || request()->routeIs('shop.show')">Shop</x-nav-link>

        <x-nav-link href="{{ $url('charity.index','/charity') }}"
                    :active="request()->routeIs('charity.index')">Charity Work</x-nav-link>
      </nav>
    </div>
  </div>
